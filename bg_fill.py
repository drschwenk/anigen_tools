...
def inpaint_bg(video, frame_n=40):
    frame_arr_data = np.load(os.path.join(t_dir,  frame_arr_dir, video.gid() + '.npy'))
    cutout_path, frame_path = make_cutout(video, frame_arr_data, frame_n)
...
def make_cutout(video, frame_arr_data, frame_n):
    ...
    ent_masks = combine_masks(video, frame_n)
    ent_masks = np.expand_dims(1 - ent_masks, 2)
...
def combine_masks(video, frame_n):
    ent_masks = []
    for ent in video.data()['characters'] + video.data()['objects']:
        ent_masks.append(np.load(os.path.join(t_dir, segmentation_dir, ent.gid() + '_segm.npy.npz'))['arr_0'][frame_n])
